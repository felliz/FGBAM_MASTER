...
LINK_BW = 10000000 #Mbps
QOS_MAXIMUM_BW = LINK_BW
...
def create_defaultQueue(socket,link_speed,db_name,current_id):
    result = insert_queue(socket,str(QOS_MAXIMUM_BW),str(link_speed),db_name,current_id)
...
def create_queue(name_port,bw,id,ovsdb_IP=None,ovsdb_Port=6640):
    ...
    result = insert_queue(s,str(QOS_MAXIMUM_BW),str(bw),db_name,current_id)
    current_id += 1
    uuid_queue_temp = result['result'][0]
    uuid_queue = uuid_queue_temp['uuid'][1]
    ...
    if not qos_in_port:
        uuid_default_queue = create_defaultQueue(s,QOS_MAXIMUM_BW-float(bw),db_name,current_id)
        current_id += 1
        result = insert_QoS(s,uuid_default_queue,str(QOS_MAXIMUM_BW),db_name,current_id)
